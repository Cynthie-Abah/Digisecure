import { View, Text, StyleSheet, Pressable, Button } from "react-native"
import AuthInputField from "../SharedComponents/AuthInputField";
import AuthBg from "../SharedComponents/AuthBg";
import { LButton } from "../SharedComponents/Button";
import { NavigationContainer } from '@react-navigation/native';
import { createNativeStackNavigator } from '@react-navigation/native-stack';
import { useState } from "react";


function Signup({navigation}) {

    const [status, setStatus] = useState('company');

  return (
    <AuthBg>
    <View style={styles.Container}>
        <View style={styles.container}>

            <View>
                <Text style={styles.welcomeText}>Create New Account</Text>
                <Text style={styles.shortText}>Please fill in the form to continue</Text>
            </View> 

            {
                status === 'personal'  ?
                ( <View style={styles.inputContainer}> 
                    <AuthInputField
                        placeholder={"Full Name"}
                        password = {false}
                    />
                    <AuthInputField
                        placeholder={"Email Address"}
                        password = {false}
                    />
    
                    <AuthInputField 
                        placeholder={"Password"}
                    />
    
                    <AuthInputField 
                        placeholder={"Confirm Password"}
                    />
                </View>) : 
                status === 'company' ?
                // company 

                (<View style={styles.inputContainer}> 
                    <AuthInputField
                        placeholder={"Full Name"}
                        password = {false}
                    />

                    <AuthInputField
                        placeholder={"Office Role"}
                        password = {false}
                    />

                    <AuthInputField
                        placeholder={"Company’s Email Address"}
                        password = {false}
                    />

                    <AuthInputField
                        placeholder={"Company’s Name"}
                        password = {false}
                    />
    
                    <AuthInputField 
                        placeholder={"Password"}
                    />
    
                    <AuthInputField 
                        placeholder={"Confirm Password"}
                    />
                </View>) : ''
            }

            <LButton> 
                Sign Up
            </LButton>

            
            <Text style={{...styles.forgotPassword, alignSelf: 'center'}}>Have An Account? <Text style={styles.signUpText} onPress={()=> navigation.navigate('Login')}>Sign In</Text></Text>
            
            
        </View>
    </View>
    </AuthBg>
  )
}





const styles = StyleSheet.create({
    container: {
        flex: 1,
        paddingVertical: 150,
        alignSelf: 'center',
        width: '100%',
    },
    welcomeText: {
      color: '#E1E6E3',
      fontSize: 30,
      fontWeight: 800,
      textAlign: 'center',
      padding: 10,
    },
    shortText: {
      color: 'rgba(225, 230, 227, .9)',
      fontSize: 17,
      fontWeight: 500,
      textAlign: 'center',
    },
    inputContainer: {
        paddingTop: 60,
        width: '100%'
    },
    forgotPassword: {
        color: 'rgba(225, 230, 227, 1)',
        fontSize: 18,
        alignSelf: 'flex-end',
        padding: 10,
        paddingHorizontal: 30,
    },
    signUpText: {
        color: ' rgba(195, 200, 213, 1)',
        fontWeight: 'bold'
      },
      Container: {
        flex: 1,
        justifyContent: 'center',
        alignItems: 'center',
      },
      welcomeText: {
        color: '#E1E6E3',
        fontSize: 30,
        fontWeight: 800,
        textAlign: 'center',
        padding: 10,
      },
      shortText: {
        color: 'rgba(225, 230, 227, .9)',
        fontSize: 17,
        fontWeight: 500,
        textAlign: 'center',
      },
});

export default Signup